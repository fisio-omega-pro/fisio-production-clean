...
const narrar = (texto: string) => {
  if (typeof window !== 'undefined' && window.speechSynthesis) {
    ...
    msg.lang = 'es-ES';
    msg.rate = 0.95; // Velocidad elegante
...
export default function AccessPage() {
  ...
  // Detectar en qué sección está el usuario para narrar
  useEffect(() => {
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (sectionId !== activeSection) {
            ...
            if (narraciones[sectionId]) narrar(narraciones[sectionId]);
          }
        }
      });
    }, { threshold: 0.5 });
  ...
  return (
    <div style={pageContainer}>
      ...
      {/* AVISO DE ACCESIBILIDAD PARA LECTORES DE PANTALLA */}
...
const loginBtn: React.CSSProperties = { background: 'none', border: '1px solid #0066ff', color: '#0066ff', padding: '8px 20px', borderRadius: '100px', fontSize: '11px', fontWeight: 800 };
const heroSection: React.CSSProperties = { paddingTop: '150px', paddingBottom: '100px', px: '20px' };
