...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    // 2. TRITURADORA DE CONTRASEÑAS (BCRYPT)
    ...
    // 4. CONFIGURACIÓN DE HORARIOS Y NEGOCIO
    ...
    if (stripe) {
      ...
      // Aplicar cupón de referido si corresponde
    ...
    // Respuesta para modo Desarrollo (sin Stripe)
    res.json({ success: true, token, dashboard_url: `/dashboard?id=${ref.id}&token=${token}` });

  } catch(e) { 
    console.error("🔥 Error Crítico en Registro:", e);
    res.status(500).json({ error: "No se pudo procesar el alta. Reintente en unos minutos." }); 
  }
});

// --- LOGIN PROFESIONAL CON VERIFICACIÓN JWT ---
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (q.empty) {
      return res.status(401).json({ success: false, error: "Clínica no registrada en nuestro sistema." });
    }

    const clinicaDoc = q.docs[0];
    const clinicaData = clinicaDoc.data();

    // Verificamos si la contraseña coincide con el hachazo (hash) guardado
    const validPassword = await bcrypt.compare(password, clinicaData.password);
    if (!validPassword) {
      return res.status(401).json({ success: false, error: "Contraseña incorrecta." });
    }

    // Emitimos la Llave Maestra (Token)
    const token = jwt.sign({ clinicId: clinicaDoc.id }, JWT_SECRET, { expiresIn: '30d' });

    console.log(`🔑 Acceso autorizado para: ${clinicaData.nombre_clinica}`);
    res.json({ success: true, token, clinicId: clinicaDoc.id, nombre: clinicaData.nombre_clinica });
