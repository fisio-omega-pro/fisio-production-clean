...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    // 2. GENERACIÓN DE SLUG (Identidad de URL)
    ...
    const newClinicData = {
      ...
      password: hashedPassword, // ✅ CONTRASEÑA CIFRADA SEGURA
    ...
    // Generamos el token de sesión inmediata
    const token = jwt.sign({ clinicId: ref.id }, JWT_SECRET, { expiresIn: '30d' });

    // 5. INTEGRACIÓN CON STRIPE (Si está activo)
    if (stripe) {
    ...
    // Respuesta si no hay Stripe (Modo Dev)
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Comparamos la clave enviada con la cifrada (hachazo)
    ...
    res.status(500).json({ error: "Error en el sistema de seguridad." }); 
  }
});
    // 2. ==========================================
