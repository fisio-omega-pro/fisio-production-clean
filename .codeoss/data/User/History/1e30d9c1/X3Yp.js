...
async function callVertexAI(contents) {
  try {
    ...
    // ✅ PIEZA REPARADA: Definimos formattedContents antes de usarla
...
app.post('/api/register', async (req, res) => {
 ...
 // ============================================================
// 🔑 MOTOR DE IDENTIDAD, SEGURIDAD Y REGISTRO (REVISADO)
// ============================================================
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    // 2. SEGURIDAD: HASHEO DE CONTRASEÑA
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(d.password, salt);

    // 3. GENERACIÓN DE SLUG ÚNICO
    let slug = d.nombre_clinica.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '');
    ...
    // 4. CONFIGURACIÓN DE STRIPE CONNECT (CUENTA EXPRESS)
    ...
    if (typeof stripe !== 'undefined' && stripe) {
      try {
        ...
        console.error("⚠️ Error creando Stripe Connect:", stripeErr.message);
        // Opcional: Decidir si continuar o lanzar error según política de negocio
      }
    }

    // 5. CONSTRUCCIÓN DEL OBJETO DE DATOS (IDENTIDAD DE CLÍNICA)
    const miCodigoReferido = 'FT-' + Math.random().toString(36).substr(2, 5).toUpperCase();
    ...
    }; // ✅ Objeto cerrado correctamente ahora.
    ...
    // 8. FLUJO DE SUSCRIPCIÓN STRIPE CHECKOUT
    if (typeof stripe !== 'undefined' && stripe) {
      const planElegido = d.plan || 'solo';
      const priceId = PLANES_STRIPE[planElegido] || PLANES_STRIPE['solo'];
    ...
    // Respuesta fallback si Stripe no está configurado
    res.json({ 
      success: true, 
      token, 
      dashboard_url: `/dashboard?id=${ref.id}&token=${token}` 
    ...
    console.error("🔥 Error Crítico en Registro:", e);
    res.status(500).json({ error: "Fallo interno en el sistema de alta." }); 
  }
});
...
// --- 💳 GENERADOR DE ENLACE DE DATOS BANCARIOS (STRIPE CONNECT EXPRESS) ---
app.post('/api/stripe/onboard', verificarToken, async (req, res) => {
  try {
    // 1. Buscamos si la clínica ya tiene un ID de Stripe Connect (creado en el registro)
    const doc = await db.collection('clinicas').doc(req.clinicId).get();
    const clinica = doc.data();
    
    let accountId = clinica.stripe_connect_id;

    // 2. Si por algún motivo no tiene ID, lo creamos ahora (Resiliencia NASA)
    if (!accountId) {
      const account = await stripe.accounts.create({
        type: 'express',
        email: clinica.email,
      ...
      await db.collection('clinicas').doc(req.clinicId).update({ stripe_connect_id: accountId });
    }

    // 3. Generamos el link de Stripe de "Cero Fricción" (Válido por 5 min)
    const host = getDynamicHost(req);
    ...
    // 4. Entregamos la URL al Dashboard para el salto
    res.json({ url: accountLink.url });

  } catch (e) {
    console.error("❌ Fallo en Stripe Connect Onboarding:", e.message);
    res.status(500).send("No se pudo conectar con la pasarela de cobros.");
  }
});
...
// ============================================================
// ⏰ 15. MOTOR CENTINELA OMEGA (MANTENIMIENTO, AVISOS Y VENTAS)
// ============================================================

app.post('/api/tasks/centinela', async (req, res) => {
  console.log("👁️ Centinela OMEGA: Iniciando patrulla integral de alto rendimiento...");
  ...
  try {
    ...
    // --- FASE A: LIMPIEZA TÉCNICA (EJECUCIÓN EN PARALELO) ---
    const expiradasSnap = await db.collection('citas')
      .where('status', '==', 'pendiente_pago')
      .where('expira_el', '<=', Timestamp.now())
      .get();

    const promesasLimpieza = expiradasSnap.docs.map(doc => 
      doc.ref.update({ 
        status: 'cancelada_por_expiracion',
        cancelado_el: Timestamp.now()
      })
    );
    ...
    // --- FASES B Y C: COMUNICACIONES ACTIVAS (SOLO EN HORARIO COMERCIAL) ---
    if (esHorarioComercial) {
      ...
      const urgentesSnap = await db.collection('citas')
        ...
        .where('expira_el', '<=', Timestamp.fromDate(proximaHora))
        .get();

      const promesasRecordatorios = urgentesSnap.docs.map(async (doc) => {
        const cita = doc.data();
        await enviarAvisoJuicioFinal(cita);
        return doc.ref.update({ recordatorio_enviado: true });
      });
      ...
      const promesasVentas = prospectosSnap.docs.map(async (doc) => {
        const p = doc.data();
        try {
          if (p.canal_primario === 'whatsapp') {
            await enviarPlantillaProspeccion(p);
            return doc.ref.update({ estado: 'wa_enviado', ultima_accion: Timestamp.now() });
          } else {
            await enviarEmailVenta(p);
            return doc.ref.update({ estado: 'email_enviado', ultima_accion: Timestamp.now() });
          }
        } catch (e) { console.error("❌ Error goteo ASG:", e.message); }
      });

      // Ejecutamos todas las comunicaciones de golpe para máxima eficiencia
      const resultadosCom = await Promise.all([...promesasRecordatorios, ...promesasVentas]);
      recordatoriosCount = urgentesSnap.size;
      prospeccionEnviada = prospectosSnap.size;
    }

    // Esperamos a que la limpieza termine
    await Promise.all(promesasLimpieza);

    res.json({ 
      success: true, 
      stats: {
        cancelaciones: expiradasSnap.size,
        avisos_enviados: recordatoriosCount,
        ventas_lanzadas: prospeccionEnviada,
        hora_local_madrid: horaMadrid + "h"
      }
    });
...
// ============================================================
// 🎯 16. MOTOR ASG (AI SALES GENERATOR) Y HELPERS DE ENVÍO
// ============================================================
...
async function enviarEmailVenta(p) {
  const promptVenta = `Eres un experto en Copywriting conductual. Redacta un email breve para el Dr. ${p.nombre_contacto} de la clínica ${p.nombre_clinica}. El objetivo es que vea cómo FisioTool elimina sus No-Shows.`;
  const result = await callVertexAI([{ role: 'user', parts: [{ text: promptVenta }] }]);
  const texto = result.text; // Usamos el objeto devuelto por nuestra función blindada
  ...
  return transporter.sendMail({
    ...
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: auto; background: #fff; border-radius: 24px; overflow: hidden; border: 1px solid #eee;">
...
async function enviarAvisoJuicioFinal(cita) {
  try {
    const urlReserva = `https://fisiotool-1050901900632.us-central1.run.app/agenda/${cita.clinic_id}`;
    await transporter.sendMail({
      from: '"Ana de FisioTool" <ana@fisiotool.com>',
      to: cita.paciente_email,
      subject: "⚠️ ÚLTIMA HORA: Tu reserva en FisioTool expira pronto",
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #ef4444; border-radius: 20px; overflow: hidden;">
          ...
          <div style="padding: 40px; color: #333;">
            ...
            <div style="text-align: center; margin-top: 30px;">
                <a href="${urlReserva}" style="background: #0066ff; color: white; padding: 15px 25px; text-decoration: none; border-radius: 50px; font-weight: bold;">ASEGURAR MI CITA AHORA ➜</a>
            </div>
          </div>
        </div>`
    });
...
app.post('/api/asg/generar-email', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: "Email ASG enviado correctamente." });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ============================================================
// 👥 17. MÓDULO MULTI-ESPECIALISTA (PROTEGIDO)
// ============================================================

app.post('/api/dashboard/add-especialista', verificarToken, async (req, res) => {
  const { nombre, especialidad, email } = req.body;
  if (!nombre) return res.status(400).send("Nombre requerido.");

  try {
    const clinicaRef = db.collection('clinicas').doc(req.clinicId);
    ...
    if (clinica.plan === 'solo' && equipoSnap.size >= 1) {
        return res.status(403).json({ error: "Límite alcanzado en Plan Solo (1 especialista)." });
    }
    if (clinica.plan === 'team' && equipoSnap.size >= 5) {
        return res.status(403).json({ error: "Límite alcanzado en Plan Team (5 especialistas)." });
    }
...
app.get('/api/dashboard/equipo', verificarToken, async (req, res) => {
  try {
    const snap = await db.collection('clinicas').doc(req.clinicId).collection('especialistas').get();
...
// ============================================================
// 🏁 18. ARRANQUE FINAL Y RUTAS SOBERANAS
// ============================================================

app.use(express.static(nextOutPath));

// 2. Manejador de rutas inteligente para exportación estática
app.get('*', (req, res, next) => {
  // Las llamadas a la API o Webhooks NO entran aquí, pasan al siguiente nivel
  if (req.path.startsWith('/api') || req.path.startsWith('/webhook') || req.path.startsWith('/stripe-webhook')) {
    return next();
  }

  // Lógica de traducción de URL a archivo .html
  let targetFile = req.path === '/' ? 'index.html' : req.path;
  
  // Si la ruta no tiene extensión (ej: /setup), le añadimos .html
  ...
  if (fs.existsSync(fullPath)) {
    ...
    // Si la página exacta no existe, Next.js se encarga desde el index (Fallback)
...
║  Arquitectura: NASA Performance (Parallel Batching)      ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
    `);
});