...
// --- LIBRERÍAS DE SEGURIDAD OMEGA ---
const bcrypt = require('bcryptjs');
const jwt = require('jsonwebtoken');

// Clave secreta (En producción debe venir del Secret Manager)
const JWT_SECRET = process.env.JWT_SECRET || 'fisiotool_supreme_secret_2026';

// 🛡️ MIDDLEWARE: EL PORTERO DEL DASHBOARD
const verificarToken = (req, res, next) => {
  const authHeader = req.headers['authorization'];
  const token = authHeader && authHeader.split(' ')[1];

  if (!token) return res.status(401).json({ error: "Acceso no autorizado. Inicie sesión." });

  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    if (err) return res.status(403).json({ error: "Sesión inválida o expirada." });
    req.clinicId = decoded.clinicId; // Inyectamos el ID real extraído del token
    next();
  });
};
// --- NUEVAS PIEZAS DE SEGURIDAD OMEGA ---
const bcrypt = require('bcryptjs');
const jwt = require('jsonwebtoken');

// Clave secreta para firmar los tokens (Idealmente en Secret Manager)
const JWT_SECRET = process.env.JWT_SECRET || 'sovereign_ferrari_secret_key_2026';

// 🛡️ MIDDLEWARE: EL PORTERO (Verifica que el fisio sea quien dice ser)
const verificarToken = (req, res, next) => {
  ...
  if (!token) {
    ...
    return res.status(403).json({ error: "Acceso denegado. Se requiere autenticación profesional." });
  }

  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    if (err) {
      console.error("❌ Token inválido o expirado.");
      return res.status(401).json({ error: "Sesión expirada. Por favor, identifícate de nuevo." });
    }
    // Inyectamos el clinicId real decodificado en la petición
    // Esto evita que un usuario use su token para ver datos de OTRA clínica
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    // ==========================================
// 🚀 9. REGISTRO DE CLÍNICA: MOTOR OMEGA (FINAL)
// ==========================================
app.post('/api/register', async (req, res) => {
  const d = req.body;
  
  try {
    // 1. PROCESAMIENTO DE SEGURIDAD E IDENTIDAD
    const emailLimpio = d.email.toLowerCase().trim();
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(d.password, salt);
    ...
    // 2. GRABACIÓN SOBERANA EN FIRESTORE
    const newClinicData = {
      nombre_clinica: d.nombre_clinica,
      slug: slug,
      email: emailLimpio,
    ...
    // 3. INTEGRACIÓN UNIFICADA DE STRIPE (MULTI-PLAN + TRIAL + REFERIDOS)
    if (stripe) {
      const planElegido = d.plan || 'solo';
      const priceId = PLANES_STRIPE[planElegido] || PLANES_STRIPE['solo'];
      ...
      // Aplicación de cupón del 50% si hay código de invitación
      if (d.codigo_invitacion) {
        sessionConfig.subscription_data.discounts = [{ coupon: 'feMDHJlj' }];
      }

      const session = await stripe.checkout.sessions.create(sessionConfig);
      console.log(`🚀 Suscripción generada (${planElegido}) para: ${d.nombre_clinica}`);
      return res.json({ success: true, token, payment_url: session.url });
    }

    // Fallback Modo Desarrollo
    return res.json({ success: true, token, dashboard_url: `/dashboard?id=${ref.id}&token=${token}` });

  } catch(e) { 
    console.error("❌ Error en Registro Maestro:", e);
    if (!res.headersSent) {
        return res.status(500).json({ error: "Fallo en el servidor: " + e.message }); 
    }
  }
});
// ============================================================
...
app.post('/stripe-webhook', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  try {
    ...
    // --- NUEVO: EVENTO DE PAGO DE FACTURA (RECOMPENSA TRAS EL TRIAL) ---
    // Este es el bloque que me pediste insertar/corregir
    if (event.type === 'invoice.paid') {
      const factura = event.data.object;

      // Solo recompensamos si es un pago real de suscripción (importe > 0)
      if (factura.subscription && factura.amount_paid > 0) {
        ...
        if (idReferente) {
          console.log(`💰 Pago de referido detectado. Recompensando a: ${idReferente}`);
          const refDoc = await db.collection('clinicas').doc(idReferente).get();
          
          if (refDoc.exists && refDoc.data().suscripcion_id) {
            // Aplicamos el cupón de 50% al amigo que invitó
            await stripe.subscriptions.update(refDoc.data().suscripcion_id, {
              coupon: 'feMDHJlj',
            });
            console.log(`🎟️ Descuento del 50% aplicado al referente con éxito.`);
          }
        }
      }
    }
