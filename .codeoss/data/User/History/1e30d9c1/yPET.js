...
app.get('/agenda/:clinicId', (req, res) => {
  ...
  if (fs.existsSync(botPath)) res.sendFile(botPath);
  else res.status(404).send("Error de montaje visual.");
});

app.get('*', (req, res, next) => {
  if (req.path.startsWith('/api') || req.path.startsWith('/webhook') || req.path.startsWith('/stripe-webhook')) return next();
  
  let sanitizedPath = req.path === '/' ? 'index.html' : `${req.path.replace(/\/$/, "")}.html`;
  const fullPath = path.join(nextOutPath, sanitizedPath.replace(/^\//, ''));
