...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    const miCodigoReferido = 'FT-' + Math.random().toString(36).substr(2, 5).toUpperCase();
// 1. CREAMOS LA CUENTA CONECTADA EN STRIPE (STRIPE CONNECT EXPRESS)
    let stripeConnectId = "";
    if (stripe) {
      const account = await stripe.accounts.create({
        type: 'express',
        email: emailLimpio, // Usamos la variable que ya tienes normalizada arriba
        capabilities: {
      ...
      console.log(`💳 Cuenta Stripe Connect creada: ${stripeConnectId}`);
    }
    const newClinicData = {
      nombre_clinica: d.nombre_clinica,
      slug: slug,
      ...
      status: 'pendiente_pago',
      aceptacion_legal: { aceptado: true, fecha: admin.firestore.Timestamp.now(), version: "1.1" },
      stripe_connect_id: stripeConnectId, // ✅ ID de la hucha guardado
      cobros_configurados: false // Se activará cuando el fisio termine el proceso
