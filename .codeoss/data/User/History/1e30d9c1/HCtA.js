...
('*', (req, res, next) => {
  // Si la ruta empieza por /api, la dejamos pasar a los errores 404 de API
  if (req.path.startsWith('/api')) return next();
  
  // Para el resto de rutas visuales
  const fileapp.get = req.path === '/' ? 'index.html' : `${req.path}.html`;
  const fullPath = path.join(nextOutPath, file);

  if (fs.existsSync(fullPath)) {
    res.sendFile(fullPath);
  } else {
    // Si la página exacta no existe, Next.js se encarga desde el index
