...
app.post('/api/register', async (req, res) => {
  ...
  try {
    // 1. TRITURAMOS LA CONTRASEÑA (Hashing)
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(d.password, salt);

    // 2. GENERACIÓN DE SLUG (Tu lógica original)
    ...
    // 3. CONFIGURACIÓN DE HORARIOS Y DATOS
    ...
    // 5. MOTOR STRIPE (Tu lógica original)
    if (stripe) {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const q = await db.collection('clinicas').where('email', '==', email.toLowerCase().trim()).limit(1).get();

    if (q.empty) return res.status(401).json({ success: false, error: "Clínica no registrada." });

    const clinicaDoc = q.docs[0];
    const clinicaData = clinicaDoc.data();

    // 1. Comparamos la clave enviada con la cifrada en la base de datos
