...
app.post('/stripe-webhook', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  try {
    ...
    } // <--- Esta llave cierra el 'else if (tipo === suscripcion)'
  } // <--- Esta llave cierra el 'if (event.type === checkout.session.completed)'

  // --- EVENTO B: PAGO DE FACTURA REAL (RECOMPENSA TRAS EL TRIAL) ---
  // Este evento se dispara el día 31, cuando Stripe cobra los 100€ de verdad.
  if (event.type === 'invoice.paid') {
    const factura = event.data.object;

    // Solo recompensamos si hay pago real y es una suscripción
    if (factura.subscription && factura.amount_paid > 0) {
      try {
        // Recuperamos la suscripción para leer la "huella" del referente
        ...
        if (idReferente && idReferente !== "") {
          console.log(`💰 Pago real recibido de referido. Premiando a: ${idReferente}`);
          
          const refDoc = await db.collection('clinicas').doc(idReferente).get();
          const refData = refDoc.data();

          if (refDoc.exists && refData.suscripcion_id) {
            // Aplicamos el cupón del 50% a la suscripción del amigo que invitó
            await stripe.subscriptions.update(refData.suscripcion_id, {
              coupon: 'feMDHJlj', // ✅ Tu cupón de 50%
            });
            console.log(`🎟️ ¡RECOMPENSA APLICADA! ${refData.nombre_clinica} pagará un 50% menos.`);
          }
        }
      } catch (errRew) {
        console.error("❌ Error en motor de recompensas:", errRew.message);
      }
    }
  }
