...
import { Sparkles, Mail, Lock, ArrowRight, ShieldCheck, KeyRound } from 'lucide-react';
  ...
  const acceder = async () => {
    try {
      ...
      if (data.success) {
        ...
        alert("Credenciales incorrectas. Verifica tu email.");
      }
    } catch (e) { alert("Error de conexión con el motor central."); }
  };
        ...
        <div style={{display:'flex', flexDirection:'column', gap:'25px'}}>
          ...
          <div style={inputGroup}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px'}}>
                ...
                <button onClick={() => alert("Ana te enviará un email para recuperar tu clave.")} style={forgotBtn}>¿La olvidaste?</button>
            </div>
...
const forgotBtn: React.CSSProperties = { background: 'none', border: 'none', color: '#0066ff', fontSize: '11px', fontWeight: 700, cursor: 'pointer' };
