...
// Definimos el estilo fuera para evitar recrearlo en cada render
const navBtnStyle = { 
...
export const AgendaView: React.FC<AgendaViewProps> = ({ onBlockSchedule }) => {
  // Asegúrate de que useCalendarLogic devuelve 'monthName', 'year', etc.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '4px', gap: '4px' }}>
            ...
            <button onClick={() => setViewDate(new Date())} style={{...navBtnStyle, fontSize: '12px', fontWeight: 700}}>Hoy</button>
      ...
      <div style={{ 
        ...
        {Array.from({ length: offset }).map((_, i) => (
          <div key={`empty-${i}`} style={{ height: '120px', background: 'rgba(255,255,255,0.01)' }} />
        ))}
