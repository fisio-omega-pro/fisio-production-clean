    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ color: THEME.colors.text.muted }}>{pacientes.length} expedientes activos sincronizados</p>
        </div>
            ...
            {pacientes.length > 0 ? pacientes.map((p) => (
              <tr key={p.id} style={{ borderBottom: `1px solid ${THEME.colors.border}` }}>
                ...
                <td style={{ padding: '20px', textAlign: 'right' }}>
                  <button style={{ background: 'none', border: 'none', color: THEME.colors.primary, cursor: 'pointer' }}><ChevronRight /></button>
                </td>
              </tr>
            )) : (
              <tr>
