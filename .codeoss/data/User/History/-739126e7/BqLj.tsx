...
export default function OnboardingPro() {
  ...
  const finalizarRegistro = async () => {
    narrar("Conectando con la pasarela segura de Stripe. Un momento, por favor.");
    ...
    try {
      ...
      if (result.payment_url) {
        ...
        alert("Error en el alta: " + result.error);
      }
    } catch (error) {
      alert("Error crítico en la conexión financiera.");
    }
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div style={inputGroup}>
                ...
                <div style={inputWrapper}><Building2 size={16} style={inputIcon}/><input style={inputField} value={formData.nombre_clinica} onChange={e => setFormData({...formData, nombre_clinica: e.target.value})} /></div>
              </div>
              <div style={inputGroup}>
                <label style={labelStyle}>Dirección completa *</label>
                <div style={inputWrapper}><MapPin size={16} style={inputIcon}/><input style={inputField} placeholder="Calle, número, ciudad..." onChange={e => setFormData({...formData, calle: e.target.value})} /></div>
              </div>
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div style={timeGrid}>
                <div style={inputGroup}><label style={labelStyle}>Precio Sesión (€)</label><div style={inputWrapper}><Euro size={14} style={inputIcon}/><input type="number" style={inputFieldShortPad} value={formData.precio} onChange={e => setFormData({...formData, precio: parseInt(e.target.value) || 0})} /></div></div>
                <div style={inputGroup}><label style={labelStyle}>Fianza (€)</label><div style={inputWrapper}><ShieldCheck size={14} style={inputIcon}/><input type="number" style={inputFieldShortPad} value={formData.fianza} onChange={e => setFormData({...formData, fianza: parseInt(e.target.value) || 0})} /></div></div>
              </div>

              <label style={labelStyle}>Banderas Rojas (Filtros IA)</label>
              <div style={tagCloud}>
                {['Accidente Tráfico', 'Suelo Pélvico', 'Bebés', 'Cirugía', 'Seguros'].map(tag => (
...
const timeGrid: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom:'15px' };
