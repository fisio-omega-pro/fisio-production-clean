...
// --- COMPONENTE PRINCIPAL (FERRARI PRO) ---
export default function OnboardingPro() {
  ...
  useEffect(() => {
    const scripts: any = {
      ...
      1: "Paso 1. Identidad Clínica. Introduce la ubicación y tus datos de acceso.",
      2: "Paso 2. Horarios comerciales y tiempos de descanso.",
      3: "Paso 3. Configuración de tarifas y protocolos de seguridad médica.",
      4: "Paso final. Confirmación de suscripción profesional de élite."
    };
    narrar(scripts[step]);
  }, [step]);
  ...
  const finalizarRegistro = async () => {
    ...
    try {
      ...
      alert("⚠️ El motor no responde. Se conectará en la fase final de integración.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={containerStyle}>
      ...
      <motion.div layout style={cardStyle}>
        <AnimatePresence mode="wait">
          ...
          {/* PASO 1: IDENTIDAD Y DIRECCIÓN */}
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <h3 style={stepTitleStyle}>Identidad Clínica</h3>
              
              <div style={inputGroup}>
                <label style={labelStyle}>Nombre Comercial de la Clínica *</label>
                <div style={inputWrapper}>
                  <Building2 size={16} style={inputIconStyle}/>
                  <input style={inputFieldWithIcon} value={formData.nombre_clinica} onChange={e => setFormData({...formData, nombre_clinica: e.target.value})} />
                </div>
              </div>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1.5fr 1fr', gap:'10px'}}>
                <div style={inputGroup}><label style={labelStyle}>Email Administrador *</label><input style={inputFieldNormal} type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
                <div style={inputGroup}>
          ...
          {/* PASO 3: REGLAS Y BANDERAS ROJAS */}
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={timeGrid}>
                <div style={inputGroup}>
                  ...
                  <div style={inputWrapper}>
                    <Euro size={14} style={inputIconStyle}/>
                    <input type="number" style={inputFieldWithIcon} value={formData.precio} onChange={e => setFormData({...formData, precio: parseInt(e.target.value) || 0})} />
                  </div>
                </div>
                <div style={inputGroup}>
                  <label style={labelStyle}>Fianza Señal (€)</label>
                  <div style={inputWrapper}>
                    <ShieldCheck size={14} style={inputIconStyle}/>
                    <input type="number" style={inputFieldWithIcon} value={formData.fianza} onChange={e => setFormData({...formData, fianza: parseInt(e.target.value) || 0})} />
                  </div>
                  <label style={labelStyle}>MÉTODOS DE PAGO ACEPTADOS</label>
<div style={tagCloud}>
  {['Efectivo', 'Tarjeta (Stripe)', 'Bizum'].map(method => (
    <div key={method} onClick={() => {/* Lógica de selección */}} style={tagStyle(formData.payMethods?.includes(method))}>
      {method}
    </div>
  ))}
</div>

<label style={labelStyle}>BANDERAS ROJAS (Triaje de Alerta Ana)</label>
                </div>
              </div>
              ...
              <div style={tagCloud}>
                {[
                  ...
                  const active = formData.flags.includes(item.id);
                  return (
                    <div key={item.id} onClick={() => toggleFlag(item.id)} style={tagStyle(active, item.color)}>
                      <item.icon size={14} color={active ? "#fff" : item.color} />
                      {item.label}
                      {active && <AlertTriangle size={12} className="animate-pulse" />}
                    </div>
                  );
                })}
              </div>
          ...
          {/* PASO 4: SUSCRIPCIÓN */}
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} style={{ textAlign: 'center' }}>
              <div style={pricingCard}>
                ...
                <div style={benefitList}>
                  ...
                  <div style={benefitItem}><CheckCircle2 size={16} color="#10b981" /> <strong>Historial por Voz:</strong> Ahorro de 5 horas de papeleo semanal.</div>
                  <div style={benefitItem}><CheckCircle2 size={16} color="#10b981" /> <strong>Agenda Soberana:</strong> Control total de huecos y bloqueos.</div>
                  ...
                  <div style={benefitItem}><CheckCircle2 size={16} color="#10b981" /> <strong>Accesibilidad Triple A:</strong> Diseñado para el talento invidente.</div>
                  <div style={benefitItem}><CheckCircle2 size={16} color="#10b981" /> <strong>Balance Real:</strong> Contabilidad y rentabilidad en tiempo real.</div>
                ...
                <div style={legalBlockStyle}>
                  <input type="checkbox" id="lCheck" checked={formData.aceptacion_legal} onChange={e => setFormData({...formData, aceptacion_legal: e.target.checked})} style={{width:'20px', height:'20px', cursor:'pointer'}} />
                  <label htmlFor="lCheck" style={{fontSize:'11px', opacity:0.6, textAlign:'left', cursor:'pointer'}}>Acepto los términos de servicio y el contrato de encargado de tratamiento de datos de salud (RGPD).</label>
                </div>
              </div>
              
              <button disabled={loading || !formData.aceptacion_legal} onClick={finalizarRegistro} style={(loading || !formData.aceptacion_legal) ? btnDisabled : btnPay}>
                {loading ? "CONECTANDO..." : "INICIAR SUSCRIPCIÓN SEGURA ➜"}
              </button>

              <button onClick={() => setStep(3)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', width: '100%', marginTop: '15px', cursor: 'pointer', fontSize: '13px' }}>Atrás: Revisar configuración</button>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
...
const cardStyle: React.CSSProperties = { width: '100%', maxWidth: '550px', background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '40px', padding: '40px', backdropFilter: 'blur(30px)', position: 'relative', zIndex: 1, boxShadow: '0 50px 100px -20px rgba(0,0,0,0.6)' };
...
const benefitList: React.CSSProperties = { display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '15px', textAlign: 'left' };
const benefitItem: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12px', color: 'rgba(255,255,255,0.7)', lineHeight: '1.4' };
