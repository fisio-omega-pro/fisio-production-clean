...
import { 
  ...
  Clock, PauseCircle, AlertTriangle, Baby, Car, Activity,
  Ticket, Check, FileText, Loader2, LucideIcon 
} from 'lucide-react';
...
export default function OnboardingSwiss() {
  ...
  const [errorMsg, setErrorMsg] = useState<string | null>(null); // Corregido: Estado añadido
  ...
  // --- FUNCIÓN FINALIZAR REGISTRO (AUDITADA Y CORREGIDA) ---
  const finalizarRegistro = async () => {
    // 1. Validación UI
    if (!formData.aceptacion_legal) { 
      setErrors(prev => ({ ...prev, legal: true })); 
      return; 
    }
    ...
    try {
      ...
      // 3. Lógica de Redirección Condicional (Tu petición)
      ...
      const message = err instanceof Error ? err.message : "Error fatal de conexión";
      setErrorMsg(message);
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" {...anim}>
              ...
              <div style={s.formGrid}>
                <Input label="NOMBRE COMERCIAL" placeholder="Ej: Clínica Avanza" value={formData.nombre} onChange={(v) => update('nombre', v)} error={errors.nombre} icon={<Building2 size={16}/>} />
                <div style={s.row2}>
                ...
                <div style={s.row3}>
                  ...
                  <Select label="PROVINCIA" value={formData.provincia} onChange={(v) => update('provincia', v)} options={PROVINCIAS} error={errors.provincia} />
                </div>
                <div style={s.divider} />
                <Input label="EMAIL ADMINISTRADOR" type="email" value={formData.email} onChange={(v) => update('email', v)} error={errors.email} icon={<Mail size={16}/>} />
                <div style={s.passWrapper}>
                  <Input 
                    label="CONTRASEÑA" type={showPass ? "text" : "password"} value={formData.password} onChange={(v) => update('password', v)} error={errors.password} icon={<Lock size={16}/>}
          ...
          {step === 3 && (
            <motion.div key="s3" {...anim}>
              ...
              <div style={s.row2}>
                <Input label="SESIÓN (€)" type="number" value={formData.precio_sesion} onChange={(v) => update('precio_sesion', Number(v))} />
                <Input label="FIANZA (€)" type="number" value={formData.fianza} onChange={(v) => update('fianza', Number(v))} />
              </div>
...
const Select = ({label, value, onChange, options, error}: {label:string, value:string, onChange:(v:string)=>void, options:string[], error?:boolean}) => (
...
const FlagItem = ({label, icon: Icon, active, onClick}: {label:string, icon:LucideIcon, active:boolean, onClick:()=>void}) => (
...
// --- ESTILOS (Tipados para evitar errores de renderizado) ---
const s: Record<string, CSSProperties> = {
  container: { minHeight: '100vh', background: '#09090b', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif', padding: '20px', position: 'relative' },
  aurora: { position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 0%, #1e1b4b 0%, #09090b 70%)', pointerEvents: 'none' },
  ...
  inputWrapper: { position: 'relative', display: 'flex', alignItems: 'center', border: '1px solid', borderRadius: '10px', background: '#09090b', overflow: 'hidden' },
  input: { width: '100%', background: 'transparent', border: 'none', padding: '10px 12px', color: '#fff', fontSize: '14px', outline: 'none' },
  select: { width: '100%', background: 'transparent', border: 'none', padding: '10px 12px', color: '#fff', fontSize: '14px', outline: 'none', cursor: 'pointer' },
  leftIcon: { position: 'absolute', left: '15px', color: '#3f3f46', display: 'flex', alignItems: 'center' },
  ...
  timeHeader: { fontSize: '10px', fontWeight: 800, color: '#a1a1aa', marginBottom: '12px', display:'flex', gap:'6px', alignItems: 'center' },
  ...
  btnPrimaryFull: { width: '100%', background: '#3b82f6', color: '#fff', border: 'none', padding: '16px', borderRadius: '10px', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginTop: '20px' },
  ...
  errorBanner: { color: '#ef4444', fontSize: '11px', textAlign: 'center', marginTop: '15px', fontWeight: 600 }
};
