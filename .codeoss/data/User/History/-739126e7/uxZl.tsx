...
export default function OnboardingSwiss() {
  ...
  // ESTA ES LA FUNCIÓN QUE FALTABA O NO SE VEÍA
  const handleNext = () => {
  ...
  const finalizarRegistro = async () => {
    ...
    try {
      ...
      alert("Error de conexión con el motor OMEGA.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 3 && (
            <motion.div key="s3" {...anim}>
              ...
              <div style={s.btnRow}><button onClick={() => setStep(2)} style={s.btnSecondary}><ArrowLeft size={20}/></button><button onClick={finalizarRegistro} disabled={loading} style={s.btnPrimary}>{loading ? "PROCESANDO..." : "ACTIVAR CUENTA"}</button></div>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
