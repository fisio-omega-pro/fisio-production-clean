        ...
        <section style={{ position: 'relative', zIndex: 1 }}>
          <AnimatePresence mode='wait'>
            ...
            >
              ...
              /* ========================================== */
              /* 🧠 MÓDULO 5: CONSULTORÍA ANA 24/7          */
              /* ========================================== */
              {activeTab === 'asistente' && (
                <div style={{ display: 'flex', flexDirection: 'column', height: '600px' }}>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '30px', background: 'rgba(0,0,0,0.2)', borderRadius: '32px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '20px' }}>
                    {anaChat.map((m, i) => (
                      <motion.div 
                      ...
                      >
                        <span style={{ color: m.role === 'ana' ? '#0066ff' : '#10b981', fontWeight: 800, fontSize: '11px', textTransform:'uppercase', marginBottom:'8px', letterSpacing:'1px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '15px', background:'rgba(255,255,255,0.02)', padding:'10px', borderRadius:'100px', border:'1px solid rgba(255,255,255,0.05)' }}>
                    <input 
                      value={anaInput} 
                      onChange={(e) => setAnaInput(e.target.value)} 
                      onKeyPress={(e) => e.key === 'Enter' && consultarAna()} 
                      placeholder="Pregunta a Ana sobre gestión o estrategia..." 
                      style={inputStyleFull()} 
                    />
                    <button onClick={consultarAna} style={actionBtnStyle('#0066ff')}>ENVIAR</button>
                  </div>
                </div>
              )}

              /* ========================================== */
              /* 🏢 MÓDULO 6: MIS SEDES Y ENLACE WHATSAPP   */
              /* ========================================== */
              {activeTab === 'sedes' && (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
                    <h2>Infraestructura Clínica</h2>
                    <button onClick={() => setShowModal('sede')} style={actionBtnStyle('#0066ff')}><PlusCircle size={18} /> Nueva Sede</button>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '30px' }}>
                    <div style={{ padding: '40px', background: 'rgba(255,255,255,0.02)', borderRadius: '32px', border: '1px solid rgba(255,255,255,0.05)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
                         <h3 style={{margin:0}}>Sede Principal</h3>
                         <span style={statusBadgeStyle('ACTIVA')}>ACTIVA</span>
                      </div>
                      
                      {/* ENLACE DE ORO (WHATSAPP) */}
                      <div style={{ marginTop: '30px', padding: '20px', background: '#05070a', borderRadius: '20px', border: '1px solid rgba(0,102,255,0.3)' }}>
                        <label style={{ fontSize: '10px', color: '#0066ff', fontWeight: 800, letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <Smartphone size={12} /> ENLACE PARA TU WHATSAPP BUSINESS
                        </label>
                        <div style={{ display: 'flex', gap: '15px', marginTop: '15px' }}>
                          <input readOnly value={`fisiotool.app/${clinicId}`} style={{ background: 'transparent', border: 'none', color: '#fff', fontSize: '13px', flex: 1, fontFamily: 'monospace' }} />
                          <Copy 
                            size={18} color="#38bdf8" style={{cursor:'pointer'}} 
                            onClick={() => { navigator.clipboard.writeText(`https://fisiotool.app/${clinicId}`); alert("✅ Enlace copiado"); }} 
                          />
                        </div>
                      </div>
                    </div>
              ...
              /* ========================================== */
              /* 💳 MÓDULO 7: HUB DE PAGOS (HÍBRIDO)        */
              /* ========================================== */
              {activeTab === 'cobros' && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
                  {/* STRIPE EXPRESS */}
                  <div style={paymentBoxStyle('#0066ff')}>
                    <div style={{ marginBottom: '20px', padding: '15px', background: 'rgba(0,102,255,0.1)', borderRadius: '16px', display: 'inline-flex' }}>
                       <CreditCard size={24} color="#0066ff" />
                    </div>
                    <h4>Stripe Express (Automático)</h4>
                    ...
                    <button onClick={conectarStripe} style={actionBtnStyle('#0066ff', true)}>CONECTAR EN 3 PASOS ➜</button>
                  </div>

                  {/* BIZUM DIRECTO */}
                  <div style={paymentBoxStyle('#10b981')}>
                    <div style={{ marginBottom: '20px', padding: '15px', background: 'rgba(16,185,129,0.1)', borderRadius: '16px', display: 'inline-flex' }}>
                       <Smartphone size={24} color="#10b981" />
                    </div>
                    <h4>Bizum Directo (Manual)</h4>
                    <p style={{ opacity: 0.5, fontSize: '13px', lineHeight: '1.6', marginBottom: '30px' }}>
                      Ana pedirá al paciente que envíe un Bizum. Tú tendrás que validar el pago manualmente.
                    </p>
                    <input placeholder="Tu número para Bizum" style={inputStyleFull()} />
                  </div>
                </div>
              )}

              /* ========================================== */
              /* 🎁 MÓDULO 8: REFERIDOS Y AJUSTES           */
              /* ========================================== */
              {activeTab === 'referidos' && (
                 <div style={{ textAlign: 'center', padding: '60px' }}>
                   <Gift size={64} color="#d4af37" style={{ marginBottom: '30px' }} />
                   <h2 style={{fontSize: '32px'}}>Premio por Recomendación</h2>
                   <p style={{ opacity: 0.5, maxWidth: '500px', margin: '0 auto' }}>Comparte tu éxito. 50% de descuento este mes para ti y tu amigo.</p>
                   <div style={{ background: 'rgba(255,255,255,0.05)', padding: '25px', borderRadius: '20px', display: 'inline-flex', gap: '20px', alignItems: 'center', marginTop: '40px', border: '1px solid rgba(212,175,55,0.3)' }}>
                     <code style={{color: '#d4af37', fontWeight: 800, fontSize: '16px'}}>fisiotool.app/invite/{clinicId}</code>
                     <Copy size={20} style={{ cursor: 'pointer' }} onClick={() => {navigator.clipboard.writeText(`fisiotool.app/invite/${clinicId}`); alert("✅ Código copiado");}} />
                   </div>
                 </div>
              )}

              {activeTab === 'ajustes' && (
                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '50px', borderRadius: '32px', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
                  <Volume2 size={48} color="#0066ff" style={{ marginBottom: '30px' }} />
                  ...
                  <div 
                    onClick={() => {
                        const newState = !narratorActive;
                        setNarratorActive(newState); 
                        if(newState) setTimeout(() => narrar("Modo lectura activado. Ana te guiará."), 100);
                    }} 
                    style={switchStyle(narratorActive)}
                  >
                    <div style={switchCircleStyle(narratorActive)} />
                  </div>
                </div>
              )}
