...
/// ============================================================
// 📝 9. REGISTRO DE NUEVA CLÍNICA (STRIPE CONNECT + OMEGA AUTH)
// ============================================================
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    // 1. NORMALIZACIÓN Y SEGURIDAD ATÓMICA
    const emailNormal = d.email.toLowerCase().trim();
    
    // TRITURADORA DE CONTRASEÑAS: Ya no guardamos texto plano
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(d.password, salt);

    // 2. GENERACIÓN DE IDENTIDAD ÚNICA (SLUG)
    ...
    // 4. CREACIÓN DE CUENTA CONECTADA (STRIPE CONNECT EXPRESS)
    // Esto crea la "hucha" donde el fisio recibirá sus fianzas directamente
    let stripeConnectId = "";
    if (stripe) {
      ...
      console.log(`💳 Hucha Stripe Connect creada para ${d.nombre_clinica}: ${stripeConnectId}`);
    }
    ...
    const newClinicData = {
      ...
      slug, 
      email: emailNormal, 
      password: hashedPassword, // ✅ Seguridad Supreme
      stripe_connect_id: stripeConnectId, // ✅ ID para transferencias directas
      pago_configurado: false, // Se marcará true cuando el fisio termine de poner su IBAN
    ...
    // 5. GENERACIÓN DE SUSCRIPCIÓN (MODO FERRARI 100/300/500)
    if (stripe) {
      ...
      const sessionConfig = {
        ...
        mode: 'subscription',
        subscription_data: {
          trial_period_days: 30, // 🎁 30 días de prueba gratis
      ...
      // Si tiene código amigo, aplicamos el cupón del 50%
