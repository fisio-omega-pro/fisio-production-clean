...
/**
 * FISIOTOOL ONLINE - BLOQUE 7: Admin, Centinela y Arranque Final
 */

// ============================================================
// 13. THE FOUNDRY: MONITORIZACIÓN GLOBAL (ADMIN)
// ============================================================
...
app.get('/api/admin/stats-globales', async (req, res) => {
    try {
        ...
        clinicasSnap.forEach(doc => {
            ...
            // Cálculo de MRR (Ingresos Recurrentes Mensuales)
            if (d.status === 'activo') {
...
// ============================================================
// 14. ANA STRATEGIC ASSISTANT (CONSULTORÍA DE NEGOCIO)
// ============================================================
...
app.post('/api/dashboard/ana-assistant', async (req, res) => {
    ...
    try {
        ...
        Analiza el negocio y da pautas de crecimiento.
        CONTEXTO: Precio sesión ${info.precio_sesion}€. Localización ${info.direcciones?.[0]?.ciudad || 'No definida'}.
        
        DUDA DEL FISIOTERAPEUTA: "${message}"
        Responde con autoridad, elegancia y visión de negocio.
        `;

        const reply = await callVertexAI([{ role: 'user', parts: promptAssistant }]);
        res.json({ success: true, reply });
    } catch (e) {
        res.status(500).send("Error en la consultoría.");
    }
});

// ============================================================
// 17. MOTOR CENTINELA (TAREAS DE MANTENIMIENTO Y VENTAS)
// ============================================================

app.post('/api/tasks/centinela', async (req, res) => {
    console.log("🕵️ Centinela: Iniciando patrulla...");
    ...
    try {
        ...
        // A. LIMPIEZA: Cancelar citas impagadas tras 12h
        ...
        for (const doc of expiradasSnap.docs) {
            ...
            console.log(`🗑️ Cita expirada: ${doc.id}`);
        }

        // B. COMUNICACIONES DE VENTA (Solo en horario comercial)
        let ventasEnviadas = 0;
        if (esHorarioComercial) {
            ...
            for (const doc of prospectosSnap.docs) {
                // Aquí se llamaría a enviarPlantillaProspeccion o similar
                await doc.ref.update({ 
                    estado: 'contactado', 
                    ultima_accion: Timestamp.now() 
                });
                ventasEnviadas++;
            }
        }
        ...
        res.json({ 
            success: true, 
            limpieza: expiradasSnap.size, 
            ventas: ventasEnviadas 
        });

    } catch (e) {
        console.error("❌ Error Centinela:", e.message);
        res.status(500).json({ error: e.message });
    }
});

// ============================================================
// 20. MOTOR VISUAL Y RUTAS CATCH-ALL (NEXT.JS SYNC)
// ============================================================
...
app.get('/agenda/:clinicId', (req, res) => {
    ...
    else res.status(404).send("Motor visual no encontrado.");
});

// Redirección Soberana: Permite refrescar páginas de Next.js
app.get('*', (req, res, next) => {
    if (req.path.startsWith('/api') || req.path.startsWith('/webhook')) return next();
    
    let targetFile = req.path === '/' ? 'index.html' : req.path;
    if (!targetFile.includes('.')) targetFile += '.html';
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`
    ╔══════════════════════════════════════════════════════════╗
    ║           🏎️  FERRARI FISIOTOOL ONLINE 2026               ║
    ╠══════════════════════════════════════════════════════════╣
    ║  Puerto: ${PORT} | Estado: OPERATIVO (GRADO OMEGA)         ║
    ║  IA Engine: Gemini Engine Activo                         ║
    ║  Seguridad: SSL/TLS + Firma de Webhooks                  ║
    ╚══════════════════════════════════════════════════════════╝
    `);
});