  ...
  return (
    <aside style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '48px', paddingLeft: '8px' }}>
        <div style={{ 
          width: '40px', height: '40px', background: THEME.colors.primary, borderRadius: '12px', 
          display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: THEME.effects.glow(THEME.colors.primary).boxShadow 
        }}>
          <Sparkles size={20} color="#fff" />
        </div>
        <span style={{ fontSize: '20px', fontWeight: 900, color: '#fff' }}>
          FISIOTOOL <span style={{ color: THEME.colors.primary }}>PRO</span>
        </span>
      </div>
      ...
      <nav style={{ flex: 1, overflowY: 'auto' }}>
        {Object.entries(items).map(([group, navItems]) => (
          <div key={group} style={{ marginBottom: '24px' }}>
            <p style={{ 
              fontSize: '10px', fontWeight: 800, color: THEME.colors.text.disabled, 
              letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '12px', paddingLeft: '12px' 
            }}>
              {group}
            </p>
            {navItems.map((item) => {
                ...
                >
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="navGlow"
                      style={{ 
                        position: 'absolute', left: 0, width: '4px', height: '18px', 
                        background: THEME.colors.primary, borderRadius: '0 4px 4px 0' 
                      }}
                    />
                  )}
                </button>
      ...
      <div style={{ paddingTop: '20px', borderTop: `1px solid ${THEME.colors.border}` }}>
        <button style={{ 
          display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', 
          background: 'none', border: 'none', color: THEME.colors.text.disabled, cursor: 'pointer' 
        }}>
