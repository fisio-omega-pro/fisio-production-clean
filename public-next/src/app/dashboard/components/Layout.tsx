...
const NavButton = React.memo(({ item, isActive, onClick }: { item: NavItemConfig; isActive: boolean; onClick: () => void }) => (
  ...
  >
    <item.icon size={18} aria-hidden="true" style={{ color: isActive ? '#0066ff' : 'inherit', filter: isActive ? 'drop-shadow(0 0 8px #0066ff80)' : 'none' }} />
  ...
  return (
    <div className="flex h-screen w-screen bg-[#030507] text-white overflow-hidden font-sans">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0a0a0a] border-r border-white/5 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-8" aria-label="Navegación principal">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#030507]">
        <header className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-[#0a0a0a]/50 backdrop-blur-md">
          <button className="lg:hidden" onClick={() => setIsMobileOpen(!isMobileOpen)} aria-label={isMobileOpen ? 'Cerrar menú' : 'Abrir menú'}><Menu aria-hidden="true" /></button>
          ...
          <div className="flex items-center gap-4 text-[10px] font-bold text-gray-600"><span>{date}</span><Bell size={16} aria-hidden="true" /></div>
        </header>
        <main className="flex-1 overflow-y-auto p-8 relative custom-scrollbar" aria-label="Contenido del dashboard">{children}</main>
      </div>
    </div>
  );
};
